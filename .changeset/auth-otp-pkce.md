...
- `OtpResult`, `OtpVerificationError`, `OtpOptionsError`, `OTP_DEFAULTS`: a single error type with a `code` discriminant — the verify-path codes share one generic user-facing message, while the issue-path codes (`challenge_invalid`, `challenge_conflict`) are safe to disambiguate — plus `attemptCount` and `cause` for logging.
