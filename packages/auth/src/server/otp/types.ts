 ...
 * re-issued, since nothing triggers a check on those. Give your adapter its
 * own cleanup for abandoned records: a scheduled job that deletes rows past
 * their expiry, or a native TTL if your store has one, such as a Postgres
 * partial index plus a cron, or Redis `EXPIRE`. Without it, rows accumulate
 * without bound.
 *
 * @public
 */
export interface OtpVerificationStore {
   ...
   * distinguish an expired leftover from a live OTP that was genuinely
   * issued moments ago. It consumes and retries the leftover, so a stale row
   * cannot permanently block re-issue for that identifier.
   */
   ...
   * `UPDATE ... SET attempts = attempts + 1`), not a read-then-write.
   * Concurrent requests that each read-then-write can each pass an
   * attempt-cap check before any of them commits, giving an attacker free
   * attempts under load.
   */
...
export interface CreateOtpAuthOptions {
   ...
   * Minimum 8, no maximum, since a longer OTP is only ever more secure.
   * Defaults to 8.
   */
  otpLength?: number
  /**
 ...
 * way in, the same way at both issue and verify time, or the two calls will
 * not resolve to the same record. See the README.
 *
 * @public
 */
export interface OtpAuth {
   ...
   *
   * On success, `data` is only `{ otpPrefix }`, a confirmation value rather
   * than the OTP itself. The plain OTP is never in this function's return
   * value; it exists only as an argument to `sendOtp`.
   *
   ...
   * already exists for this pair, so mint a fresh verifier), or
   ...
   * concurrent request, or `'unexpected'` if `store` threw (see
   * {@link OtpResult}). All of these carry the same generic
   * `error.message`, so branch on `error.code`. Never show `error.message`
