...
the same file — the plain OTP and its hash — which is exactly the kind of
...
_Avoid_: token (this is the ambiguity the source had — `token` meant both
this and the **OTP hash**, in the same file), code (ambiguous with
**challenge**)
